...
device_specs = {
    ...
    # Bluetooth-connected variant of "SpaceMouse Wireless" reports a
    # different product ID (0xC63A) than the USB-receiver variant above
    # (0xC62E) -- confirmed via easyhid.Enumeration().find() against a real
    # device (product_string="SpaceMouse Wireless BT", vendor 3Dconnexion).
    # Axis/button layout copied from the USB variant on the assumption the
    # underlying HID report format matches; verify against real teleop and
    # adjust byte offsets below if any axis reads flipped/wrong.
